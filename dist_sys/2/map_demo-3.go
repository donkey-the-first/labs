...
func main() {
    ...
    map_name := "demo-distributed-map-3"
    ...
    for i := 0; i < numClients; i++ {
        ...
        go func(clientID int) {
            ...
            for k := 0; k < iterations; k++ {
                err := hzMap.Lock(ctx, "key")
                if err != nil {
                    log.Printf("Клієнт %d: Помилка блокування: %v", clientID, err)
                    continue
                }
                defer hzMap.Unlock(ctx, "key")
                ...
                val++
                _, err = hzMap.Put(ctx, "key", val)
                if err != nil {
                    log.Printf("Клієнт %d: Помилка запису значення: %v", clientID, err)
                    continue
                }
            }
